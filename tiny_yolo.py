...
conv1_biases = tf.Variable(tf.zeros([16], dtype=tf.float32))
conv2_weights = tf.Variable(tf.truncated_normal([3, 3, 16, 32], stddev=0.1, seed=SEED, dtype=tf.float32))
conv2_biases = tf.Variable(tf.constant(0.1, shape=[32], dtype=tf.float32))
conv3_weights = tf.Variable(tf.truncated_normal([3, 3, 32, 64], stddev=0.1,seed=SEED, dtype=tf.float32))
conv3_biases = tf.Variable(tf.zeros([64], dtype=tf.float32))
conv4_weights = tf.Variable(tf.truncated_normal([3, 3, 64, 128], stddev=0.1, seed=SEED, dtype=tf.float32))
conv4_biases = tf.Variable(tf.constant(0.1, shape=[128], dtype=tf.float32))
conv5_weights = tf.Variable(tf.truncated_normal([3, 3, 128, 256], stddev=0.1, seed=SEED, dtype=tf.float32))
conv5_biases = tf.Variable(tf.zeros([256], dtype=tf.float32))
conv6_weights = tf.Variable(tf.truncated_normal([3, 3, 256, 512], stddev=0.1, seed=SEED, dtype=tf.float32))
conv6_biases = tf.Variable(tf.constant(0.1, shape=[512], dtype=tf.float32))
conv7_weights = tf.Variable(tf.truncated_normal([3, 3, 512, 1024], stddev=0.1, seed=SEED, dtype=tf.float32))
conv7_biases = tf.Variable(tf.zeros([1024], dtype=tf.float32))
conv8_weights = tf.Variable(tf.truncated_normal([3, 3, 1024, 1024], stddev=0.1, seed=SEED, dtype=tf.float32))
conv8_biases = tf.Variable(tf.constant(0.1, shape=[1024], dtype=tf.float32))
conv9_weights = tf.Variable(tf.truncated_normal([3, 3, 1024, 1024], stddev=0.1, seed=SEED, dtype=tf.float32))
conv9_biases = tf.Variable(tf.constant(0.1, shape=[1024], dtype=tf.float32))
conv10_weights = tf.Variable(tf.truncated_normal([3, 3, 1024, 1024], stddev=0.1, seed=SEED, dtype=tf.float32))
conv10_biases = tf.Variable(tf.constant(0.1, shape=[1024], dtype=tf.float32))

fc1_weights = tf.Variable(tf.truncated_normal([IMAGE_SIZE * IMAGE_SIZE //4096  * 1024, 4096], stddev=0.1, seed=SEED, dtype=tf.float32))
fc1_biases = tf.Variable(tf.constant(0.1, shape=[4096], dtype=tf.float32))
fc2_weights = tf.Variable(tf.truncated_normal([4096, 4096], stddev=0.1, seed=SEED, dtype=tf.float32))
fc2_biases = tf.Variable(tf.constant(0.1, shape=[4096], dtype=tf.float32))
fc3_weights = tf.Variable(tf.truncated_normal([4096, S*S*(B*5+CLASSES)], stddev=0.1, seed=SEED, dtype=tf.float32))
fc3_biases = tf.Variable(tf.constant(0.1, shape=[S*S*(B*5+CLASSES)], dtype=tf.float32))
...
def model(data, train=False):
  ...
  conv = tf.nn.conv2d(pool, conv6_weights, strides=[1, 1, 1, 1], padding='SAME')
  conv_bias = tf.nn.bias_add(conv, conv6_biases)
  lrelu = tf.maximum(alpha*conv_bias, conv_bias)

  pool = tf.nn.max_pool(lrelu, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')

  conv = tf.nn.conv2d(pool, conv7_weights, strides=[1, 1, 1, 1], padding='SAME')
  conv_bias = tf.nn.bias_add(conv, conv7_biases)
  lrelu = tf.maximum(alpha*conv_bias, conv_bias)


  conv = tf.nn.conv2d(lrelu, conv8_weights, strides=[1, 1, 1, 1], padding='SAME')
  conv_bias = tf.nn.bias_add(conv, conv8_biases)
  lrelu = tf.maximum(alpha*conv_bias, conv_bias)

  conv = tf.nn.conv2d(lrelu, conv9_weights, strides=[1, 1, 1, 1], padding='SAME')
  conv_bias = tf.nn.bias_add(conv, conv9_biases)
  lrelu = tf.maximum(alpha*conv_bias, conv_bias)

  conv = tf.nn.conv2d(lrelu, conv10_weights, strides=[1, 1, 1, 1], padding='SAME')
  conv_bias = tf.nn.bias_add(conv, conv10_biases)
  lrelu = tf.maximum(alpha*conv_bias, conv_bias)

  fc1_shape = lrelu.get_shape().as_list()
  reshape = tf.reshape(lrelu, [fc1_shape[0], fc1_shape[1] * fc1_shape[2] * fc1_shape[3]])

  fc1_hidden = tf.matmul(reshape, fc1_weights) + fc1_biases
  fc1_out = tf.maximum(alpha*fc1_hidden, fc1_hidden)

  fc2_hidden = tf.matmul(fc1_out, fc2_weights) + fc2_biases
  fc2_out = tf.maximum(alpha*fc2_hidden, fc2_hidden)

  return tf.matmul(fc2_out, fc3_weights) + fc3_biases
